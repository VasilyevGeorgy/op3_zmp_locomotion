...
void op3_zmp_inv_kin::initialization(KDL::Frame pelvis_pose){
  ...
  rleg_min_pos_limit_.push_back(-0.01);	rleg_max_pos_limit_.push_back(90.0); // kn_p /////////////////////////////////////////
  ...
  lleg_min_pos_limit_.push_back(-0.01);	lleg_max_pos_limit_.push_back(90.0); // kn_p
...
bool op3_zmp_inv_kin::movePelvis(KDL::Frame leg_des_pose, Eigen::VectorXd &leg_des_joint_pos_, std::string legType){
  ...
  if (legType == "right"){
    pseudo = KDL::Frame(KDL::Rotation::RPY(-roll_,-pitch_,-yaw_),
                           KDL::Vector(rfoot_pose.p.x()-dx, rfoot_pose.p.x()-dy, rfoot_pose.p.x()-dz)
                        );
    ...
    if(legType == "left"){
      pseudo = KDL::Frame(KDL::Rotation::RPY(-roll_,-pitch_,-yaw_),
                             KDL::Vector(lfoot_pose.p.x()-dx, lfoot_pose.p.x()-dy, lfoot_pose.p.x()-dz)
                          );
      ...
      if (ik_pose_err != 0)
      {
        ROS_WARN("RIGHT LEG IK ERROR : %s", lleg_ik_pos_solver->strError(ik_pose_err));
...
void op3_zmp_inv_kin::goToInitialPose(KDL::Frame pelvis_des_pose){
  ...
  pelvis_pose = KDL::Frame(
                    ...
                    KDL::Vector(0.0,0.0,0.34) // z - max height
                       );
  ...
  while (ros::ok()){
    pelvis_pose.p.data[2] += dz;
    std::cout<<"pelvis_pose.p.z() = "<<pelvis_pose.p.z()<<std::endl;

    this->movePelvis(pelvis_pose, rleg_joint_pos_, "Right");
    this->movePelvis(pelvis_pose, lleg_joint_pos_, "Left");
